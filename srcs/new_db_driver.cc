...
#include "client.h"
#include "client_postgresql.h" // 추가된 코드: PostgreSQL 클라이언트 헤더 포함
#include "config.h"
#include "env.h"
#include "types.h"
...
int main(int argc, char *argv[]) {
  ...
  // 주석: 여러 DBClient를 관리하기 위해 벡터로 변경
  std::vector<std::unique_ptr<client::DBClient>> db_clients; // 추가된 코드

  // 주석: PostgreSQL 클라이언트 생성 및 초기화
  {
    std::string db_name = "PostgreSQL"; // 추가된 코드
    std::string startup_cmd = config["startup_cmd"].as<std::string>(); // 추가된 코드
    auto postgres_client = std::make_unique<client::PostgreSQLClient>(); // 추가된 코드
    postgres_client->initialize(config); // 추가된 코드
    db_clients.emplace_back(std::move(postgres_client)); // 추가된 코드
  }

  /* This is where the testcase data is written into */
  constexpr size_t kMaxInputSize = 0x100000;
  u8 *buf = (u8 *)malloc(
      kMaxInputSize);  // this is the maximum size for a test case! set it!
  if (!buf) { // 추가된 코드: malloc 실패 체크
  ...
  // 주석: 각 DB 클라이언트에 대해 서버가 살아있는지 확인하고, 아니면 시작
  for (auto &db_client : db_clients) { // 추가된 코드
    if (!db_client->check_alive()) { // 수정된 코드
      std::string startup_cmd = config["startup_cmd"].as<std::string>(); // 수정된 코드
      system(startup_cmd.c_str()); // 수정된 코드
      sleep(5); // 서버 시작 대기
    }
  }
  ...
  while ((len = __afl_next_testcase(buf, kMaxInputSize)) > 0) {
    ...
    // 주석: 각 DB 클라이언트에 대해 테스트 케이스 실행
    for (auto &db_client : db_clients) { // 추가된 코드
      db_client->prepare_env(); // 추가된 코드

      client::ExecutionStatus status = db_client->execute((const char *)buf, len); // 추가된 코드

      __afl_area_ptr[0] = 1;
      /* report the test case is done and wait for the next */

      if (status == client::kServerCrash) {
        while (!db_client->check_alive()) { // 수정된 코드
          // Wait for the server to be restarted.
          sleep(5);
        }
      }
      db_client->clean_up_env(); // 추가된 코드
      __afl_end_testcase(status); // 추가된 코드
    }
  }
